...
test_that("Testing facto_ca()", {
  ...
  # Active individuals
  expect_equivalent(observed_ca$row$coord, as.data.frame(expected_ca$row$coord))
  expect_equivalent(observed_ca$row$cos2, as.data.frame(expected_ca$row$cos2))
  expect_equivalent(observed_ca$row$contrib, as.data.frame(expected_ca$row$contrib))
  expect_equal(observed_ca$row$inertia, expected_ca$row$inertia)

  # Supplementary individuals
  expect_equivalent(observed_ca$row.sup$coord, as.data.frame(expected_ca$row.sup$coord))
  expect_equivalent(observed_ca$row.sup$cos2, as.data.frame(expected_ca$row.sup$cos2))

  # Active Variables
  expect_equivalent(observed_ca$col$coord, expected_ca$col$coord)
  expect_equal(observed_ca$col$cor, expected_ca$col$cor)
  expect_equivalent(observed_ca$col$cos2, expected_ca$col$cos2)
  expect_equivalent(observed_ca$col$contrib, as.data.frame(expected_ca$col$contrib))
  expect_equal(observed_ca$col$inertia, expected_ca$col$inertia)

  # Supplementary Variables
  expect_equivalent(observed_ca$col.sup$coord, expected_ca$col.sup$coord)
  expect_equivalent(observed_ca$col.sup$cos2, as.data.frame(expected_ca$col.sup$cos2))
