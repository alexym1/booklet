...
description: >
  Start here if this is your first time using lavander.
  This document deals with the differences between FactoMineR and lavander.
vignette: >
...
`lavander` was designed to return same result as `FactoMineR` but with a more modern and tidyverse-friendly syntax. This document aims to show the differences between the two packages.
...
```{r}
# Get eigvalues and eigvectors with lavander
X_active <- X[-1, -10]
...
```{r}
# Get principal components with lavander
...
```{r}
# Get individual cos2 with lavander
...
```{r}
# Get individual contributions with lavander
...
```{r}
# Get supplementary individuals with lavander
...
```{r}
# Get supplementary individuals cos2 with lavander
...
```{r}
# Get variable coordinates with lavander
...
```{r}
# Get variable cos2 with lavander
...
```{r}
# Get variable contributions with lavander
...
```
```{r}
# Get supplementary CONTINUOUS variables coordinates with lavander
...
```{r}
# Get supplementary variables cos2 with lavander
